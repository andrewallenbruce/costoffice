...
```{r}
cardio <- search_datasets(specialty = "cardiology") |> download_dataset()
...
search_datasets(specialty = "vascular surgery") |> 
  ...
  tidytable::summarise(avg_min_price = round(mean(new_price_min), 2),
                       avg_max_price = round(mean(new_price_max), 2),
                       avg_mode_price = round(mean(new_price_mode), 2),
                       avg_copay_min = round(mean(new_copay_min), 2),
                       avg_copay_max = round(mean(new_copay_max), 2),
                       avg_copay_mode = round(mean(new_copay_mode), 2),
                       .by = c(state, new_top_code)) |> 
  tidytable::select(-new_top_code) |> 
  tidytable::drop_na() |> 
  knitr::kable()
```
...
search_datasets(specialty = "cardiology") |> 
  ...
  tidytable::summarise(avg_min_price = round(mean(est_price_min), 2),
                       avg_max_price = round(mean(est_price_max), 2),
                       avg_mode_price = round(mean(est_price_mode), 2),
                       avg_copay_min = round(mean(est_copay_min), 2),
                       avg_copay_max = round(mean(est_copay_max), 2),
                       avg_copay_mode = round(mean(est_copay_mode), 2),
                       .by = c(state, est_top_code)) |> 
  tidytable::select(-est_top_code) |> 
  tidytable::drop_na() |> 
  knitr::kable()
```
